...
public slots:
  ...
  void SetTexture(QPixmap, int num, bool isMipmap = true,
                  bool isReversed = true, bool SetSize = 0);
  ...
  void ReloadFilter();
  void ReloadShader();
  // Load the filter weights from given file.
  void LoadFilter(const QString &filter_file);

  void ToggleInfor(bool info) {
    m_ShowInfo = info;
    updateGL();
  }
...
signals:
  ...
  /// @details This signal is used for slider control, since slider is
  /// disabled for current use, this signal is not connected to any slot,
  /// which makes it useless This routine is only kept for further use
  ...
  /// @details This signal is used for slider control, since slider is
  /// disabled for current use, this signal is not connected to any slot. This
  /// routine is only kept for further use
  /// @param Value Changed amount of rotation
  void zTranslationChanged(float value);
  /// @brief Qt signal fires when the mouse position has been changed
  /// @details Since is the a 1D Spline, only the x value is passed to dispaly
  /// a plain in qtglBSplineClassifier
...
private:
  ...
  /// @details This bool variable will trigger whether the OpenGL
  /// Viewport/ModelView/Projection matrix needs to be reloaded.
  bool m_CameraChanged;
  /// @brief The Origin(0,0,0) screen depth position under current camera
  /// coordinates
  ...
  bool m_toggle_Point;
  QTimer *m_AutoTimer;
  FilterWeights filter_weights_handler_;
