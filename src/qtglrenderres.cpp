...
// QSize qtglRenderRes::minimumSizeHint() const { return QSize(50, 50); }

// QSize qtglRenderRes::sizeHint() const { return QSize(300, 300); }
...
// m_CameraChanged will be set to true to enable the update loading of
// m_GLmodelview/m_GLprojection/m_GLviewport parameters
...
void qtglRenderRes::initializeGL() {
  ...
  //            GL_SPHERE_MAP); // Set The Texture Generation Mode For S To
  //            Sphere
  //                            // Mapping (NEW)
  //  glTexGeni(GL_T, GL_TEXTURE_GEN_MODE,
  //            GL_SPHERE_MAP); // Set The Texture Generation Mode For T To
  //            Sphere
  //                            // Mapping (NEW)
  ...
  m_FilterFile = QString("./GLSL/blur.txt");

  //  m_VShaderFile = QString(".//../src/Basic.vsh");
  //  m_FShaderFile = QString(".//../src/Basic.fsh");
  //  m_FilterFile = QString(".//../src/blur.txt");

  //  m_VShaderFile = QString(":Basic.vsh");
  //  m_FShaderFile = QString(":Basic.fsh");
  //  m_FilterFile =
  //  QString("O:/YouyouCodes/TAMU_reserach/Lp_Filer/src/blur.txt");
  LoadShader(m_VShaderFile, m_FShaderFile);
  LoadFilter(m_FilterFile);

  InitializeShaderParam();
  SetShaderParame();
}

void qtglRenderRes::ReloadFilter() {
  LoadFilter(m_FilterFile);
  ShaderProgram->setUniformValue("row", filter_weights_handler_.Height());
  ShaderProgram->setUniformValue("col", filter_weights_handler_.Width());
  ShaderProgram->setUniformValueArray(
      "FilterWeight", filter_weights_handler_.GetWeights().data(),
      filter_weights_handler_.GetWeights().size(), 1);
  show();
  updateGL();
}
...
void qtglRenderRes::SetShaderParame() {
  ...
  ShaderProgram->setUniformValue("tex_real", 0);
  //  ShaderProgram->setUniformValue("tex_SM", 1);
  //  ShaderProgram->setUniformValue("tex_DI_Dark", 2);
  //  ShaderProgram->setUniformValue("tex_DI_Bright", 3);
  //  ShaderProgram->setUniformValue("tex_SI", 4);
  //  ShaderProgram->setUniformValue("tex_env", 5);
  //  ShaderProgram->setUniformValue("is_Video", 0);
  ShaderProgram->setUniformValue("LP", (float)m_dist);
  ...
  //  ShaderProgram->setUniformValue("toggle_ShaAmbCos",
  //  (int)m_toggle_ShaAmbCos); ShaderProgram->setUniformValue("toggle_Mirror",
  //  m_toggle_Mirror); ShaderProgram->setUniformValue("toggle_Point",
  //  m_toggle_Point); ShaderProgram->setUniformValue("SM_Quality",
  //  m_SM_QUality); ShaderProgram->setUniformValue("Cartoon_sha",
  //  m_CartoonSha); ShaderProgram->setUniformValue("light_dir", -mMousePos);
  ShaderProgram->setUniformValue("row", filter_weights_handler_.Height());
  ShaderProgram->setUniformValue("col", filter_weights_handler_.Width());
  ShaderProgram->setUniformValueArray(
      "FilterWeight", filter_weights_handler_.GetWeights().data(),
...
void qtglRenderRes::paintGL() {
  //    glGetIntegerv(GL_VIEWPORT, m_GLviewport);           // Retrieves The
  //    Viewport Values (X, Y, Width, Height)
  ...
  // Load new ModelView/Projection/Viewport parameter if there is any change of
  // camera
  //        if(m_CameraChanged)
  //            LoadGLParameters();

  // basic OpenGL rutine, clear the screen, and load the identity for
  // m_GLmodelview
  ...
  if (m_ShowInfo) {
    ...
    //    glActiveTexture(GL_TEXTURE0);
    QString xy_pos = QString("pos: (%1,%2)")
                         .arg(mMousePos.x(), 0, 'f', 3)
                         .arg(mMousePos.y(), 0, 'f', 3);
    renderText(mMousePos.x(), mMousePos.y() + 0.01, mMousePos.z(), xy_pos,
               m_font);

    QString z_pos = QString("pow: %1").arg(mMousePos.z(), 0, 'f', 3);
    //        qDebug()<<z_pos;

    renderText(mMousePos.x(), mMousePos.y() - 0.04, mMousePos.z(), z_pos,
               m_font);
...
void qtglRenderRes::mousePressEvent(QMouseEvent *event) {
  ...
  if (!(event->modifiers() & Qt::ControlModifier)) {
    if (event->buttons() & Qt::LeftButton) {
      ...
      winY =
          (float)m_GLviewport[3] - winY; // Subtract The Current Mouse Y
                                         // Coordinate From The Screen Height.

      GLdouble posX, posY, posZ;
      //            qDebug()<<mMousePos;
      // Find the position of click in 3D, with reference to the plane defined
      // by origin and look_at direction
      gluUnProject(winX, winY, m_ZeroZ, m_GLmodelview, m_GLprojection,
                   m_GLviewport, &posX, &posY,
                   &posZ); // The new position of the mouse
      mMousePos.setX(posX);
      mMousePos.setY(posY);
      ShaderProgram->setUniformValue("light_dir", -mMousePos);
    }
...
void qtglRenderRes::LoadGLParameters() {
  ...
  glGetIntegerv(
      GL_VIEWPORT,
      m_GLviewport); // Retrieves The Viewport Values (X, Y, Width, Height)
  glGetDoublev(GL_MODELVIEW_MATRIX,
               m_GLmodelview); // Retrieve The Modelview Matrix
  glGetDoublev(GL_PROJECTION_MATRIX,
               m_GLprojection); // Retrieve The Projection Matrix

  // Not know why, but it solves the problem, maybe some issue with QT
  if (width() < height())
    m_GLviewport[1] = -m_GLviewport[1];

  // After this function is called, set the CameraChanged to be false to prevent
  // recall of this function
  m_CameraChanged = false;
  // calculate the depth value of the origin under current camera setting
  gluProject(mMousePos.x(), mMousePos.y(), mMousePos.z(), m_GLmodelview,
             m_GLprojection, m_GLviewport, &m_ZeroX, &m_ZeroY, &m_ZeroZ);
}
...
void qtglRenderRes::keyPressEvent(QKeyEvent *e) {
  ...
  case Qt::Key_Space:
    ...
    if (xRot == 0 && yRot == 0 && zRot == 0 && xTrans == 0 && yTrans == 0) {
// different Z value under different set-up pf m_GLprojection matrix
#ifdef m_GLprojection_MODEL
      zTrans = 0;
#else
...
void qtglRenderRes::SetTexture(QPixmap TextureMap, int num, bool isMipmap,
                               bool isReversed, bool SetSize) {
  ...
  if (isMipmap)
    options = options | QGLContext::MipmapBindOption |
              QGLContext::LinearFilteringBindOption;
  // if the image has to be reversed, it means this is an image, in order not to
  // make any changes to the GLSL part of checking alpha channle, I set this
  // bind type to be GL_RGB
  if (isReversed) {
    bind_type = GL_RGBA;
  ...
  m_textures[num] =
      bindTexture(m_TextureMap, GL_TEXTURE_2D, bind_type, options);
...
void qtglRenderRes::SetDepthValue(double dep) {
  ...
  ShaderProgram->setUniformValue("LP", m_dist);
  show();
  updateGL();
}
...
// some problem here with compilation, I am still suspecting it is realted to
// glactivetexture. if this function is defined as input QVector3D, then it
// crashed.
...
void qtglRenderRes::SetBGTexture(QPixmap img, int option) {
  ...
  glActiveTexture(GL_TEXTURE0);
  //    SetTexture(img, 0, 0,
  //    QGLContext::NoBindOption|QGLContext::MipmapBindOption|QGLContext::LinearFilteringBindOption);
...
void qtglRenderRes::SetLenseTexture(QPixmap img, int option) {
  ...
  // if I do not put this active texture here, then the rendtext() of Qt will
  // not work, I think it is related to how active texture works, or some bug
  // with Qt rendertext()
  glActiveTexture(GL_TEXTURE0);
  glDisable(GL_TEXTURE0);
...
void qtglRenderRes::SetDIDarkTexture(QPixmap img, int option) {
...
void qtglRenderRes::SetDIBrightTexture(QPixmap img, int option) {
...
void qtglRenderRes::SetFGTexture(QPixmap img, int option) {
  deleteTexture(m_textures[4]);
  glActiveTexture(GL_TEXTURE4);
  SetTexture(img, 4, option & 1, option & 2);
  show();
...
void qtglRenderRes::SetEnvTexture(QPixmap img, int option) {
  deleteTexture(m_textures[5]);
  glActiveTexture(GL_TEXTURE5);
  SetTexture(img, 5, option & 1, option & 2);
  show();
...
void qtglRenderRes::LoadFilter(const QString &filter_file) {
  if (!filter_weights_handler_.LoadFromFile(filter_file.toStdString())) {
    QMessageBox::critical(this, "Filter loading error",
                          "Failed to load filter from " + filter_file);
  }
}
